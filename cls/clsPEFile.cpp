		...
		do
		{
			...
			while (pIAT->u1.Ordinal != 0 && !(pIAT->u1.Ordinal & IMAGE_ORDINAL_FLAG32))
			{
				...
				if(pImportName != NULL && pImportName->Name != NULL)
				{
                    ...
                    newAPI.APIName = QString::fromLatin1((const char*)((pImportHeader->Name - dwVAOfImportSection) + dwImportSectionOffset)).append("::").append(QString::fromLatin1((const char*)pImportName->Name));
